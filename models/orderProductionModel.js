...
const orderProductionSchema = mongoose.Schema({
  ...
  qty: {
    type: Number,
    required: [true, "qty OrderProduction required!"],
  },
  ordername: {
    type: String,
    default: "",
...
orderProductionSchema.pre("save", async function (next) {
  if (this.isNew && !this.name) {
    const Model = mongoose.model("OrderProduction", orderProductionSchema);
...
exports.OrderProductionModel = mongoose.model("OrderProduction", orderProductionSchema);
